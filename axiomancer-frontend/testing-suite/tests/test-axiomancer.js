...
async function testAxiomancerApp() {
  ...
  try {
    ...
    // Verify the image is using object-fit: contain (shows full image)
    if (imageInfo.objectFit === 'contain') {
      console.log('✅ Image uses object-fit: contain - full artwork visible');
    } else {
      console.log('⚠️  Warning: Image may be cropped - object-fit is not "contain"');
    }
    
    // Check if image maintains reasonable static size
    const staticSizeCheck = await imageElement.evaluate((img) => {
      const imageIsStaticSize = img.getBoundingClientRect().height < window.innerHeight * 0.9;
      return {
        isStaticSize: imageIsStaticSize,
        viewportHeight: window.innerHeight,
        imageHeight: img.getBoundingClientRect().height
      };
    });
    
    console.log(`   Viewport height: ${staticSizeCheck.viewportHeight}px`);
    console.log(`   Image height: ${Math.round(staticSizeCheck.imageHeight)}px`);
    
    if (staticSizeCheck.isStaticSize) {
      console.log('✅ Image displays at static size (not filling entire screen)');
    } else {
      console.log('⚠️  Image may be filling entire screen');
    }
