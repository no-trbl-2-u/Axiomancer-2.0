...
router.post('/save', authenticateToken, (req: AuthenticatedRequest, res: Response): void => {
  void (async (): Promise<void> => {
    try {
      ...
      const characterState = req.body as Record<string, unknown>;
...
router.get('/load', authenticateToken, (req: AuthenticatedRequest, res: Response): void => {
  void (async (): Promise<void> => {
    try {
      const userId = req.user?.userId;
      if (!userId) {
...
router.get('/exists', authenticateToken, (req: AuthenticatedRequest, res: Response): void => {
  void (async (): Promise<void> => {
    try {
      const userId = req.user?.userId;
      if (!userId) {
...
router.delete('/delete', authenticateToken, (req: AuthenticatedRequest, res: Response): void => {
  void (async (): Promise<void> => {
    try {
      const userId = req.user?.userId;
      if (!userId) {
